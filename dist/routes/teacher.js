    ...
    try {
        ...
        const todayClasses = classes.filter(c => {
            ...
            try {
                ...
                return schedule.some((s) => s.day === todayName);
            }
            catch {
                return false;
            }
        ...
        // Students count
        const totalStudentsEnrolled = await db_1.default.enrollment.count({
...
router.put('/submissions/:submissionId/grade', async (req, res) => {
    ...
    try {
        const submission = await db_1.default.assignmentSubmission.update({
        ...
        if (questions && Array.isArray(questions)) {
            for (const q of questions) {
                ...
                if (q.options && Array.isArray(q.options)) {
                    await db_1.default.questionOption.createMany({
